...
namespace Activatar.Server
{
    public class ProductKeyPublisher
    {
        ...
        public string GenerateProductKey(string keyString, Guid guid)
        {
            try
            {
                SHA1CryptoServiceProvider sha1 = new SHA1CryptoServiceProvider();
                byte[] keyStringBytes = sha1.ComputeHash(Encoding.ASCII.GetBytes(keyString));
                byte[] sig = _cryptoService.SignData(keyStringBytes, sha1);
                byte[] key = new byte[32];
                byte[] iv = new byte[16];

                Array.Copy(sig, key, 32);
                Array.Copy(sig, 128, iv, 0, 16);

                RijndaelManaged crypt = new RijndaelManaged();
                byte[] hidden = crypt.CreateEncryptor(key, iv).TransformFinalBlock(sig, 0, sig.Length);

                byte[] keyBytes;
                using (MemoryStream ms = new MemoryStream())
                {
                    ms.Write(hidden, 0, 5);
                    ms.Write(guid.ToByteArray(), 5, 2); ;
                    ms.Write(hidden, hidden.Length-8, 8);

                    keyBytes = ms.ToArray();
                }

                string k = Base32Converter.ToBase32String(keyBytes);

                return string.Format("{0}-{1}-{2}-{3}-{4}", k.Substring(0, 5), k.Substring(5, 5), k.Substring(10, 5), k.Substring(15, 5), k.Substring(20, 5));
            }
            catch (Exception ex)
            {
